        ...
        <div className="relative overflow-hidden [mask-image:linear-gradient(90deg,transparent,#000_8%,#000_92%,transparent)]">
          <div className="marquee-track flex w-max items-center gap-12">
            {row.map((brand, i) => (
              <div
                key={brand.name + i}
                className="flex items-center gap-2 text-lg font-semibold text-muted-2"
              >
                {brand.logo ? (
                  <Media
                    image={brand.logo}
                  ...
                  <span className="whitespace-nowrap font-display tracking-tight opacity-70 grayscale transition hover:opacity-100">
                    {brand.name}
                  </span>
                )}
              </div>
